...
import {
  ...
  LeavePdfDestination,
} from './reports/leave-request.report';
import { PushNotificationsService } from '../push-notifications/push-notifications.service';

export const finalLeaveDocumentKinds = (
  businessDays: number,
): LeavePdfDestination[] =>
  businessDays > 3 ? ['DIRECTOR', 'FINAL'] : ['FINAL'];
...
export class LeaveRequestsService {
  ...
  async create(requesterId: string, dto: CreateLeaveRequestDto) {
    ...
    if (!activeJob?.area_id || !employee.regional_id) {
      throw new BadRequestException(
        'El empleado no tiene área y regional activas configuradas.',
      );
    }
    ...
    if (businessDays <= 0) {
      throw new BadRequestException(
        'El rango debe contener al menos un día laboral.',
      );
    }
    ...
    if (overlapping) {
      throw new BadRequestException(
        'Ya existe una licencia activa que se cruza con ese rango.',
      );
    }
    ...
    const regionalManager =
      await this.regionalManagerService.findActiveByRegional(
        employee.regional_id,
      );
    const areaApproval =
      await this.approvalRoutingService.resolveAreaOrMainManager(
        employee.id,
        activeJob.area_id,
        employee.regional_id,
      );
    ...
    if (!isMainOffice && !regionalManager) {
      throw new BadRequestException(
        'No existe un jefe regional activo para la regional del empleado.',
      );
    }
    ...
    const saved = await this.requestRepository.save(
      this.requestRepository.create({
        ...
        regionalManagerEmployeeId: isMainOffice
          ? null
          : regionalManager!.employee_id,
  ...
  async updateDocuments(
    ...
    if (
      ...
      throw new BadRequestException(
        'No puede modificar documentos de una licencia finalizada.',
      );
    }
    this.validateDocuments(dto.documents || []);
    const allowed = new Set(
      this.requiredDocumentCodes(request.reasonType, request.relationship),
    );
    for (const document of dto.documents || []) {
      if (!allowed.has(document.code))
        throw new BadRequestException(
          'El documento no corresponde a esta licencia.',
        );
      const previous = request.documents?.find(
        (item) => item.code === document.code,
      );
      const extension = this.extensionForMime(document.mimeType);
      const filePath = this.storageService.saveBase64File(
        document.base64,
        `leave-requests/${request.id}`,
        `${document.code}-${Date.now()}.${extension}`,
      );
      if (previous) {
        this.storageService.deleteFile(previous.filePath);
        previous.originalName = document.name;
        previous.mimeType = document.mimeType;
  ...
  async reviewByManager(
    ...
    const request = await this.requestRepository.findOne({
      where: { id },
      relations: { employee: true, regional: true, documents: true },
    });
    if (!request)
      throw new NotFoundException('Solicitud de licencia no encontrada.');
    ...
    const assignedId = isRegionalStep
      ? request.regionalManagerEmployeeId
      : request.areaManagerEmployeeId;
    if ((!isRegionalStep && !isAreaStep) || assignedId !== reviewer.id) {
      throw new ForbiddenException(
        'No tiene autorización para revisar esta licencia.',
      );
    }
  ...
  async findLiaisonInbox(requesterId: string) {
    const employee = await this.resolveEmployee(requesterId);
    const access = await this.regionalManagerService.getHrLiaisonAccess(
      employee.id,
    );
    ...
    return requests.map((request) => ({
      ...
      missingDocumentCodes:
        this.withDocumentStatus(request).missingDocumentCodes,
  ...
  async reviewByLiaison(
    ...
    if (
      ...
      throw new BadRequestException(
        'La licencia ya no está pendiente del enlace de RR. HH.',
      );
    }
    const liaisons =
    ...
    if (!liaisons.some((item) => item.employee_id === reviewer.id))
      throw new ForbiddenException(
        'No tiene permiso para revisar esta licencia.',
      );
    if (dto.status === LeaveRequestStatus.APPROVED)
      this.assertDocumentsComplete(request);
    ...
    await sendRequestNotification(
      ...
      dto.status === LeaveRequestStatus.APPROVED
        ...
        : 'El enlace regional de Recursos Humanos denegó su solicitud.',
      dto.observation ? [`Observación: ${dto.observation}`] : [],
    );
    await this.pushNotifications.sendToEmployee(
      request.employee.id,
  ...
  async getDirectorAccess(requesterId: string) {
    ...
    return {
      ...
      employeeId: employee.id,
      roleLabel:
        director.employee_id === employee.id ? 'Director General' : null,
    };
  }
  ...
  async reviewByHr(
    ...
    try {
      ...
      if (!request)
        throw new NotFoundException('Solicitud de licencia no encontrada.');
      request.employee = await runner.manager.findOneByOrFail(Employee, {
      ...
      if (
        ...
        throw new BadRequestException(
          'La solicitud ya no está pendiente de Recursos Humanos.',
        );
      }
      if (dto.status === LeaveRequestStatus.APPROVED)
        this.assertDocumentsComplete(request);
      ...
      if (dto.status === LeaveRequestStatus.REJECTED) {
        ...
        if (director.employee_id === request.employeeId) {
          throw new BadRequestException(
            'El Director General no puede autorizar su propia licencia.',
          );
        }
      ...
      const completedRequest = await this.getById(request.id);
      await this.notifyAfterHrReview(completedRequest);
  ...
  async reviewByDirector(
    ...
    try {
      ...
      const completedRequest = await this.getById(request.id);
      await this.notifyFinalDecision(completedRequest);
      return completedRequest;
    } catch (error) {
      await runner.rollbackTransaction();
      throw error;
  ...
  async generatePdf(
    ...
    if (destination === 'FINAL') {
      if (
        ...
        throw new BadRequestException(
          'La resolución final aún no está disponible.',
        );
      }
    } else if (request.businessDays <= 3) {
      throw new BadRequestException(
        'Las licencias de tres días o menos no generan oficio para Dirección.',
      );
    } else if (request.hrStatus !== LeaveRequestStatus.APPROVED) {
      throw new BadRequestException(
        'El oficio para Dirección aún no está disponible.',
      );
    }
    ...
    return {
      ...
      filename:
        destination === 'FINAL'
          ? `resolucion-final-${request.requestNumber}.pdf`
          : `oficio-rrhh-direccion-${request.requestNumber}.pdf`,
      pdf: this.printerService.createPdf(
        buildLeaveRequestReport(request, destination),
      ),
    };
  }
  ...
  async getDocument(
    ...
    const isAssignedManager =
      request.regionalManagerEmployeeId === employee.id ||
      request.areaManagerEmployeeId === employee.id;
  ...
  async getById(id: string) {
    const request = await this.requestRepository.findOne({
      where: { id },
      relations: {
        employee: { jobRecords: { position: true, modality: true } },
        area: true,
        regional: true,
        hrEmployee: true,
  ...
  private async findInbox(
    ...
    if (reviewer === 'HR') {
      builder.andWhere(
        '(request.liaisonReviewRequired = false OR request.liaisonStatus = :liaisonApproved OR request.status <> :liaisonPending)',
        {
    ...
    if (directorId)
      builder.andWhere('request.directorEmployeeId = :directorId', {
        directorId,
      });
    ...
    if (directorId)
      scope.where('request.directorEmployeeId = :directorId', { directorId });
    const pendingStageForReviewer =
      reviewer === 'HR'
        ? LeaveRequestStage.HR_REVIEW
        : LeaveRequestStage.DIRECTOR_REVIEW;
    const [pending, approved, rejected, directorPending, completed] =
      await Promise.all([
        scope
          .clone()
          .andWhere(
        ...
        scope
          ...
          .andWhere(
            reviewer === 'HR'
              ...
              : 'request.directorStatus = :approved',
        ...
        scope
          ...
          .andWhere(
            reviewer === 'HR'
              ...
              : 'request.directorStatus = :rejected',
            { rejected: LeaveRequestStatus.REJECTED },
          )
          .getCount(),
        scope
          .clone()
          .andWhere('request.stage = :directorStage', {
            directorStage: LeaveRequestStage.DIRECTOR_REVIEW,
          })
          .getCount(),
        scope
          .clone()
          .andWhere('request.stage = :completedStage', {
            completedStage: LeaveRequestStage.COMPLETED,
          })
          .getCount(),
      ]);
    return {
      ...
      stats: {
        ...
        directorPending,
        completed,
        total: await scope.clone().getCount(),
      },
    };
  }

  private applyInboxStatus(
    builder: any,
    status: string,
    reviewer: 'HR' | 'DIRECTOR',
  ) {
    const statusColumn =
      reviewer === 'HR' ? 'request.hrStatus' : 'request.directorStatus';
    const pendingStage =
      reviewer === 'HR'
        ? LeaveRequestStage.HR_REVIEW
        : LeaveRequestStage.DIRECTOR_REVIEW;
    if (status === 'pending')
      return builder
        .andWhere(`${statusColumn} = :filterStatus`, {
          filterStatus: LeaveRequestStatus.PENDING,
        })
        .andWhere('request.stage = :pendingStage', { pendingStage });
    if (status === 'approved')
      return builder.andWhere(`${statusColumn} = :filterStatus`, {
        filterStatus: LeaveRequestStatus.APPROVED,
      });
    if (status === 'rejected')
      return builder.andWhere(`${statusColumn} = :filterStatus`, {
        filterStatus: LeaveRequestStatus.REJECTED,
      });
    return builder;
  }
  ...
  private async applyUnpaidLeaveImpact(request: LeaveRequest, manager: any) {
    ...
    const firstIndex = periods.findIndex(
      (period: EmployeeVacationPeriod) =>
        request.startDate <= period.accreditationDate &&
        request.endDate >= period.startDate,
    );
    ...
    if (firstIndex >= 0) {
      for (const period of periods.slice(firstIndex)) {
        ...
        period.accreditationDate = this.addCalendarDays(
  ...
  private validateLegalRequest(
    ...
    if (dto.reasonType === LeaveReasonType.DEATH) {
      if (dto.relationship === LeaveRelationship.SELF) {
        throw new BadRequestException(
          'El parentesco "El empleado" no aplica para una licencia por fallecimiento.',
        );
      }
  ...
  private withDocumentStatus(request: LeaveRequest) {
    const requiredDocumentCodes = this.requiredDocumentCodes(
      request.reasonType,
      request.relationship,
    );
    const received = new Set(
      (request.documents || []).map((item) => item.code),
    );
    const missingDocumentCodes = requiredDocumentCodes.filter(
      (code) => !received.has(code),
    );
    return {
      ...request,
      requiredDocumentCodes,
      missingDocumentCodes,
      documentsComplete: missingDocumentCodes.length === 0,
  ...
  private assertDocumentsComplete(request: LeaveRequest) {
    ...
    if (!status.documentsComplete)
      throw new BadRequestException(
        'El empleado debe completar los documentos de respaldo antes de aprobar la licencia.',
      );
  }
  ...
  private async countBusinessDays(startDate: string, endDate: string) {
    ...
    const holidayDates = new Set(
      holidays.map((item) => String(item.date).slice(0, 10)),
    );
    let count = 0;
    for (
      let date = this.dateAtNoon(startDate);
      date <= this.dateAtNoon(endDate);
      date.setDate(date.getDate() + 1)
    ) {
      const iso = this.serializeDate(date);
      if (date.getDay() !== 0 && date.getDay() !== 6 && !holidayDates.has(iso))
        count += 1;
    }
    return count;
  }
  ...
  private async resolveEmployee(requesterId: string) {
    ...
    if (!user?.employee)
      throw new NotFoundException(
        'No se encontró el empleado del usuario autenticado.',
      );
  ...
  private async assertHrAccess(requesterId: string) {
    ...
    if (!count)
      throw new ForbiddenException(
        'No tiene permiso para administrar licencias.',
      );
  }
  ...
  private async getMainDirector() {
    ...
    return {
      ...
      employee: {
        email: main.employeeEmail,
        firstName: main.employeeName,
      } as Employee,
    };
  }
  ...
  private async assertMainDirector(employeeId: string) {
    const director = await this.getMainDirector();
    if (director.employee_id !== employeeId)
      throw new ForbiddenException('Acceso exclusivo del Director General.');
  }

  private async notifyAfterHrReview(request: LeaveRequest) {
    if (
      request.hrStatus === LeaveRequestStatus.REJECTED ||
    ...
    await Promise.all([
      sendRequestNotification(
        request.employee.email,
        `${request.requestNumber} aprobada por RR. HH.`,
        this.employeeName(request.employee),
        'Recursos Humanos aprobó su licencia de más de tres días laborales. Ahora está pendiente de la decisión del Director General.',
      ),
      sendRequestNotification(
        director.employee.email,
        `Licencia ${request.requestNumber} pendiente`,
        this.employeeName(director.employee),
        'Tiene una solicitud de licencia de cuatro o más días laborales pendiente de revisión por Dirección General.',
        [
          `Empleado: ${this.employeeName(request.employee)}`,
          `Días laborales: ${request.businessDays}`,
        ],
        'https://sigerh.senasa.gob.hn/leave-director-requests/pending',
      ),
    ]);
  }
  ...
  private async notifyFinalDecision(request: LeaveRequest) {
    ...
    // PdfPrinter shares font state: serialize creation so attachments cannot corrupt each other.
    if (request.status === LeaveRequestStatus.APPROVED) {
      for (const destination of finalLeaveDocumentKinds(request.businessDays)) {
        attachments.push({
          filename:
            destination === 'FINAL'
              ? `resolucion-final-${request.requestNumber}.pdf`
    ...
    await sendRequestNotification(
      ...
      this.employeeName(request.employee),
      request.status === LeaveRequestStatus.APPROVED
        ? 'Su solicitud de licencia fue aprobada definitivamente.'
        : 'Su solicitud de licencia fue denegada.',
      [request.directorObservation || request.hrObservation].filter(
        Boolean,
      ) as string[],
  ...
  private assertDecision(status: LeaveRequestStatus) {
    if (
      ![LeaveRequestStatus.APPROVED, LeaveRequestStatus.REJECTED].includes(
        status,
      )
    )
      throw new BadRequestException('Solo puede aprobar o denegar.');
  }
  ...
  private normalizeDate(value: string) {
    ...
    if (
      ...
      Number.isNaN(this.dateAtNoon(normalized).getTime())
    )
  ...
  private dateAtNoon(value: string) {
  ...
  private addCalendarDays(value: string, days: number) {
    const date = this.dateAtNoon(value);
