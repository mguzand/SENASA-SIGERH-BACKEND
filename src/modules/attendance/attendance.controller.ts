...
export class AttendanceController {
  constructor(
    ...
    private readonly pdfReport: MonthlyAttendancePdfReport,
    private readonly excelReport: MonthlyAttendanceExcelReport,
  ) {}
  @Get('employees') getEmployees(@Query('regionalId') regionalId?: string) {
    return this.service.getEmployeeOptions(regionalId);
  }
  @Get('monthly-report') getMonthly(
  ...
  @Get('monthly-report/excel') async getMonthlyExcel(
    ...
    response.setHeader(
      'Content-Disposition',
      `attachment; filename="reporte-asistencia-${regional}-${query.month}-${query.year}.xlsx"`,
    );
    response.send(excel);
  }
}
