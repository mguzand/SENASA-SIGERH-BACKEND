...
import {
  LeaveRequestStatus,
  LeaveRequestType,
} from '../leave-requests/enums/leave-request.enums';
...
export function resolveAttendanceCode(input: {
  ...
  const priority: AttendanceIncident['kind'][] = [
    'VACATION',
    'GOVERNMENT_VACATION',
    'HOLIDAY',
    'UNPAID_LEAVE',
  ...
  const incident = priority
    .map((kind) => incidents.find((item) => item.kind === kind))
    .find(Boolean);
  if (incident) {
    const base = {
      entry: input.mark?.entry || null,
      exit: input.mark?.exit || null,
      source: 'ADMINISTRATIVE' as const,
    };
    if (incident.kind === 'VACATION')
      return {
        ...base,
        code: 'V',
        status: 'VACATION',
    ...
    const permit = String(incident.permitType || '')
      .trim()
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '');
    const permitCodes: Record<string, string> = {
      personal: 'PP',
      oficial: 'PO',
      'medico ihss': 'CM',
      'medico privado': 'MP',
      'servicio de salud publica': 'SSP',
      'otros permisos': 'OP',
      'otros pases': 'OP',
    };
    ...
    return {
      ...
      status: 'PERMIT',
      description: incident.description,
      requiresClassification: !code,
    };
  }
  const scheduled = {
    scheduledEntry: input.schedule?.startTime || null,
    scheduledExit: input.schedule?.endTime || null,
  };
  if (!input.mark)
    return {
      code: null,
      status: input.isWeekend ? 'NON_WORKING_DAY' : 'NO_DATA',
      description: input.isWeekend
        ? 'Fin de semana sin novedad'
        : 'Sin información de asistencia',
      entry: null,
      exit: null,
      ...scheduled,
      source: null,
    };
  if (!input.mark.entry && !input.mark.exit)
    return {
      code: null,
      status: 'MISSING_BOTH',
      description: 'Sin marcación de entrada ni salida',
      entry: null,
      exit: null,
      ...scheduled,
      source: 'BIOMETRIC',
  ...
  if (!input.mark.entry)
    return {
      ...
      description: 'Sin marcación de entrada',
      entry: null,
      exit: input.mark.exit,
      ...scheduled,
      source: 'BIOMETRIC',
    };
  if (!input.mark.exit)
    return {
      code: null,
      status: 'MISSING_EXIT',
      description: 'Sin marcación de salida',
      entry: input.mark.entry,
      exit: null,
      ...scheduled,
      source: 'BIOMETRIC',
    };
  if (!input.schedule)
    return {
      code: null,
      status: 'NO_SCHEDULE',
      description: 'Marcación no evaluable: empleado sin horario',
      entry: input.mark.entry,
      exit: input.mark.exit,
      ...scheduled,
      source: 'BIOMETRIC',
  ...
  const late =
    minutes(input.mark.entry) >
    minutes(input.schedule.startTime) + LATE_TOLERANCE_MINUTES;
  return {
    code: late ? 'LT' : 'X',
    status: late ? 'LATE' : 'PRESENT',
    description: late ? 'Llegada tardía' : 'Marcación normal',
    entry: input.mark.entry,
    exit: input.mark.exit,
    ...scheduled,
    source: 'BIOMETRIC',
...
export class AttendanceService {
  ...
  constructor(
    ...
    private readonly permits: Repository<EmployeeExitPermit>,
    @InjectRepository(LeaveRequest)
    private readonly leaves: Repository<LeaveRequest>,
  ...
  async ensurePermissionComponent() {
    ...
    const existing = await this.components.findOne({
      where: { system_id: systemId, description: 'Reporte de asistencia' },
    });
    if (existing) return existing;
    const last = await this.components.findOne({
      where: { system_id: systemId },
      order: { orden: 'DESC' },
    });
    return this.components.save(
  ...
  async getEmployeeOptions(regionalId?: string) {
    ...
    if (regionalId)
      query.andWhere('employee.regional_id = :regionalId', { regionalId });
    return (await query.getMany()).map((employee) => ({
      id: employee.id,
  ...
  async getMonthlyReport(
    filter: MonthlyAttendanceFilterDto,
    ...
    if (filter.regionalId)
      query.andWhere('employee.regional_id = :regionalId', {
    ...
    for (
      ...
      offset += BIOMETRIC_BATCH_SIZE
    ) {
      await Promise.all(
        employees
          .slice(offset, offset + BIOMETRIC_BATCH_SIZE)
          .map(async (employee) => {
            if (!employee.biometric_id) {
              warnings.push(
                `${this.fullName(employee)} no tiene identificación biométrica.`,
              );
              return;
            }
            if (!employee.schedule) {
              warnings.push(
                `${this.fullName(employee)} no tiene horario asignado.`,
              );
              return;
            }
            try {
              const raw = await this.watchesService.getReporteMarcaciones({
                userid: employee.biometric_id,
                mes: filter.month,
                anio: filter.year,
                horaEntrada: employee.schedule.startTime,
                horaSalida: employee.schedule.endTime,
              });
              ...
              for (const row of raw || []) {
                ...
                if (date)
                  byDate.set(date, {
                    entry: row.Hora_Ingreso || null,
                    exit: row.Hora_Salida || null,
                  });
              }
    ...
    return {
      period: {
        month: filter.month,
        year: filter.year,
        monthName: MONTH_NAMES[filter.month - 1],
      },
      regional: filter.regionalId
        ? {
            id: filter.regionalId,
            name: employees[0]?.regional?.name || 'Regional seleccionada',
          }
        : { id: null, name: 'Todas las regionales' },
      days,
      employees: employees.map((employee) => ({
        id: employee.id,
        biometricId: employee.biometric_id || null,
        name: this.fullName(employee),
        regional: employee.regional?.name || 'Sin regional',
        organizationalUnit:
          employee.jobRecords?.[0]?.area?.name || 'Sin unidad asignada',
        ...
        days: Object.fromEntries(
          days.map((day) => [
            ...
            resolveAttendanceCode({
              isWeekend: ['Sábado', 'Domingo'].includes(day.weekday),
              schedule: employee.schedule
                ? {
                    startTime: employee.schedule.startTime,
                    endTime: employee.schedule.endTime,
                ...
                : null,
              mark: marks.get(employee.id)?.get(day.date),
              incidents:
                incidents.get(`${employee.id}|${day.date}`) ||
                incidents.get(`*|${day.date}`),
            }),
          ]),
        ),
        observations: !employee.biometric_id
    ...
    vacations.forEach((request) =>
      request.days.forEach((day) =>
        add(request.employee_id, day.date, {
          kind: 'VACATION',
          description: 'Vacación aprobada',
        }),
      ),
    );
    permits.forEach((permit) =>
      this.dateRange(
        String(permit.exit_date).slice(0, 10),
        String(permit.end_date || permit.exit_date).slice(0, 10),
      ).forEach((date) =>
        add(permit.employee_id, date, {
          kind: 'PERMIT',
          description: permit.description || `Permiso ${permit.permit_type}`,
          permitType: permit.permit_type,
        }),
      ),
    );
    leaves.forEach((leave) =>
      this.dateRange(leave.startDate, leave.endDate).forEach((date) =>
        add(leave.employeeId, date, {
          kind:
            leave.type === LeaveRequestType.UNPAID
              ? 'UNPAID_LEAVE'
              : 'PAID_LEAVE',
          description:
            leave.type === LeaveRequestType.UNPAID
  ...
  private normalizeWatchDate(value: unknown): string | null {
  ...
  private fullName(employee: Employee) {
    return [
      employee.firstName,
      employee.middleName,
      employee.lastName,
