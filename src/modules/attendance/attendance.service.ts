...
export class AttendanceService {
  ...
  private async loadIncidents(employeeIds: string[], start: string, end: string) {
    ...
    const [vacations, permits, leaves, holidays, governmentDays] = employeeIds.length ? await Promise.all([
      ...
      this.permits.createQueryBuilder('permit').where('permit.employee_id IN (:...ids)', { ids: employeeIds }).andWhere('permit.status = :approvedStatus AND permit.hr_status = :approvedHrStatus', { approvedStatus: ExitPermitStatus.APPROVED, approvedHrStatus: ExitPermitStatus.APPROVED }).andWhere('permit.exit_date <= :end AND COALESCE(permit.end_date, permit.exit_date) >= :start', { start, end }).getMany(),
    ...
    permits.forEach((permit) => this.dateRange(String(permit.exit_date).slice(0, 10), String(permit.end_date || permit.exit_date).slice(0, 10)).forEach((date) => add(permit.employee_id, date, { kind: 'PERMIT', description: permit.description || `Permiso ${permit.permit_type}`, permitType: permit.permit_type })));
